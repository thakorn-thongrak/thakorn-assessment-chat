...
- **Unread badge** ที่ sidebar — เทียบ id ข้อความล่าสุดของแต่ละห้องกับ id ที่แอดมินเคยอ่านล่าสุด (เก็บใน Redis, `id` เป็นเลขนับรวมทุกห้อง เลยเทียบตัวเลขตรง ๆ ได้) พอเปิดห้องแชทหรือมีข้อความใหม่เข้ามาตอนเปิดอยู่ จะ mark read ให้อัตโนมัติผ่าน `/api/read`
...
| `GET /api/conversations` | เว็บ polling ทุก 2.5 วิ ตลอดเวลาที่เปิดหน้าเว็บ | คืนรายชื่อ/รูป/ข้อความล่าสุด/`unread` ของทุกคนที่เคยทักมา ให้ sidebar โชว์ |
| `POST /api/read` | เว็บเรียกตอนเปิดห้องแชท และทุกครั้งที่ poll เจอข้อความใหม่ขณะเปิดห้องอยู่ | บันทึกว่าแอดมินอ่านห้องนี้ถึงข้อความล่าสุดแล้ว (เคลียร์ unread badge) |
